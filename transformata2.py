...
#przefiltrowany = ag.pasmowoprzepustowy(sygnal, czestProbkowania, 20, 30)
...
#plt.subplot(2, 1, 2)
#plt.plot(t, przefiltrowany, color='r')
#plt.ylim([-12, 12])
#plt.xlabel("t[s]", size=18)
#plt.ylabel("A[-]", size=18)
#plt.show()
...
sygnal1 = A2*np.sin (2* np.pi*czestotliwosc2*t1)
trans = ag.FFT(sygnal1)

plt.subplot(2, 1, 1)
plt.plot(t1, sygnal1, color='r')
plt.ylim(-4, 4)
plt.xlabel("t[s]", size=18)
plt.ylabel("A[-]", size=18)
plt.subplot(2, 1, 2)
